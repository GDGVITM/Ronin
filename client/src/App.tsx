...
function App() {
  ...
  const [isMuted, setIsMuted] = useState(() => {
    try {
      return localStorage.getItem("bgMuted") === "true";
    } catch {
      return false;
    }
  });
  ...
  const toggleMute = () => {
    setIsMuted((prev) => {
      ...
      try {
        localStorage.setItem("bgMuted", String(nextMuted));
      } catch {}
      if (audioRef.current) {
    ...
    <>
      ...
      {showMuteBtn && (
        <button
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.background = "#1a120e";
            e.currentTarget.style.transform = "translateY(-2px)";
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.background = "#110b08";
      ...
      <Routes>
        ...
        <Route
          path="/"
          element={
            <LandingPage onVideoIntroDone={() => setVideoIntroDone(true)} />
          }
        />
        <Route
          path="/login"
