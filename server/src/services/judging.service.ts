...
export async function runAgainstProblem(problemId: string, language: string, code: string) {
  ...
  let passed = 0;
  const details: { input: string; expected: string; got: string; pass: boolean; isHidden: boolean }[] = [];
  ...
  for (const testCase of problem.testCases) {
    try {
      ...
      details.push({
        input: testCase.input,
        expected: testCase.expected.trim(),
        got: `Execution error: ${(err as Error).message}`,
        pass: false,
        isHidden: testCase.isHidden,
      });
    }
